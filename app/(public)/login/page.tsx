"use client";

import { AuthLoginBackdrop } from "@/components/auth/auth-login-backdrop";
import { AuthSocialDivider } from "@/components/auth/auth-social-divider";
import { GoogleSignInButton } from "@/components/auth/google-sign-in-button";
import { GuestOnly } from "@/components/auth/require-auth";
import { FormInputField } from "@/components/common";
import { LoaderMini } from "@/components/common/loader-mini";
import OwnerLogin from "@/components/lottie/OwnerLogin.json";
import { useAuth } from "@/hooks/use-auth";
import { ApiError } from "@/lib/api";
import { isGoogleAuthConfigured } from "@/lib/google-auth";
import { useI18n } from "@/lib/i18n";
import {
  createLoginSchema,
  type LoginFormValues,
} from "@/lib/validation/auth-schemas";
import { appRoutes } from "@/routes/app-routes";
import { isAdminUser } from "@/services/auth.service";
import { zodResolver } from "@hookform/resolvers/zod";
import { goeyToast, gooeyToast } from "goey-toast";
import Lottie from "lottie-react";
import Link from "next/link";
import { useMemo, useState } from "react";
import { useForm } from "react-hook-form";

function EmailIcon() {
  return (
    <svg
      width="17"
      height="17"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <rect x="3" y="5" width="18" height="14" />
      <path d="m4 7 8 6 8-6" />
    </svg>
  );
}

function LockIcon() {
  return (
    <svg
      width="17"
      height="17"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <rect x="4" y="10" width="16" height="11" />
      <path d="M8 10V7a4 4 0 0 1 8 0v3" />
    </svg>
  );
}

function EyeIcon({ hidden }: { hidden: boolean }) {
  return (
    <svg
      width="17"
      height="17"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="1.8"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="M2 12s3.5-6 10-6 10 6 10 6-3.5 6-10 6S2 12 2 12Z" />
      <circle cx="12" cy="12" r="2.5" />
      {hidden ? <path d="m4 4 16 16" /> : null}
    </svg>
  );
}

function CheckIcon() {
  return (
    <svg
      width="16"
      height="16"
      viewBox="0 0 24 24"
      fill="none"
      stroke="currentColor"
      strokeWidth="2"
      strokeLinecap="round"
      strokeLinejoin="round"
      aria-hidden="true"
    >
      <path d="m5 12 4 4L19 6" />
    </svg>
  );
}

export default function LoginPage() {
  const { signIn, signInWithGoogle } = useAuth();
  const { t } = useI18n();
  const [showPassword, setShowPassword] = useState(false);
  const [googleBusy, setGoogleBusy] = useState(false);
  const schema = useMemo(() => createLoginSchema(t), [t]);

  const {
    control,
    handleSubmit,
    formState: { isSubmitting },
  } = useForm<LoginFormValues>({
    resolver: zodResolver(schema),
    defaultValues: { email: "", password: "" },
    mode: "onTouched",
  });

  const onSubmit = handleSubmit(async (values) => {
    try {
      const result = await signIn({
        email: values.email.trim(),
        password: values.password,
      });
      const isAdmin = isAdminUser(result.user);
      goeyToast.success(t("login.successTitle"), {
        description: isAdmin ? t("login.successAdmin") : t("login.successUser"),
      });

      window.location.assign(result.redirectTo);
    } catch (error) {
      const message =
        error instanceof ApiError ? error.message : t("login.errorFallback");
      if (
        error instanceof ApiError &&
        error.status === 403 &&
        /verify your email/i.test(message)
      ) {
        gooeyToast.error("Email Verification Required", {
          description: message,
        });

        const params = new URLSearchParams({ email: values.email.trim() });
        window.location.assign(`${appRoutes.verifyEmail}?${params.toString()}`);
        return;
      }
      gooeyToast.error(t("login.errorTitle"), {
        description: message,
      });
    }
  });

  async function handleGoogleSignIn(payload: { accessToken: string }) {
    setGoogleBusy(true);
    try {
      const result = await signInWithGoogle(payload);
      const isAdmin = isAdminUser(result.user);
      gooeyToast.success(t("login.successTitle"), {
        description: isAdmin ? t("login.successAdmin") : t("login.successUser"),
      });

      window.location.assign(result.redirectTo);
    } catch (error) {
      const message =
        error instanceof ApiError ? error.message : t("login.errorFallback");
      gooeyToast.error(t("login.errorTitle"), {
        description: message,
      });
    } finally {
      setGoogleBusy(false);
    }
  }

  const features = [
    t("login.featureListings"),
    t("login.featureProfile"),
    t("login.featureSecurity"),
  ];

  return (
    <GuestOnly>
      <AuthLoginBackdrop>
        <div className="page-shell flex flex-1 items-center py-10 md:py-14">
          <div className="mx-auto grid w-full max-w-5xl overflow-hidden rounded-none border border-light-border/90 bg-light-surface/95 shadow-lg backdrop-blur-sm dark:border-dark-border/90 dark:bg-dark-surface/95 dark:shadow-dark-lg md:grid-cols-[0.9fr_1.1fr]">
            <aside className=" auth-login-panel z-1 relative hidden overflow-hidden border-e border-light-border p-8 dark:border-dark-border md:flex md:flex-col md:justify-between lg:p-10">
              <div className=" absolute inset-0 -z-2 w-full h-full ">
                  <Lottie animationData={OwnerLogin} className=" w-full h-full" />
              </div>
            </aside>

            <div className="p-6 sm:p-8 lg:p-12">
              
              <p className="mt-6 text-xs font-semibold uppercase tracking-[0.2em] text-primary-600 dark:text-primary-500 md:mt-0">
                {t("login.eyebrow")}
              </p>
              <h1 className="mt-2 text-3xl font-semibold tracking-tight text-light-text dark:text-dark-text">
                {t("login.title")}
              </h1>
             

              <form onSubmit={onSubmit} className="mt-8 space-y-5" noValidate>
                <FormInputField
                  control={control}
                  name="email"
                  id="email"
                  type="email"
                  label={t("login.email")}
                  placeholder={t("login.emailPlaceholder")}
                  autoComplete="email"
                  startIcon={<EmailIcon />}
                  tone="light"
                />

                <FormInputField
                  control={control}
                  name="password"
                  id="password"
                  type={showPassword ? "text" : "password"}
                  label={t("login.password")}
                  placeholder={t("login.passwordPlaceholder")}
                  autoComplete="current-password"
                  startIcon={<LockIcon />}
                  endIcon={
                    <button
                      type="button"
                      onClick={() => setShowPassword((current) => !current)}
                      aria-label={
                        showPassword
                          ? t("login.hidePassword")
                          : t("login.showPassword")
                      }
                      className="grid h-8 w-8 place-items-center rounded-none text-light-muted transition hover:bg-primary-50 hover:text-primary-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/25 dark:text-dark-muted dark:hover:bg-primary-500/10 dark:hover:text-primary-500"
                    >
                      <EyeIcon hidden={showPassword} />
                    </button>
                  }
                  tone="light"
                />

                <div className="flex justify-end">
                  <Link
                    href={appRoutes.forgotPassword}
                    className="text-sm font-semibold text-primary-600 transition hover:text-primary-700 hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/25 dark:text-primary-500 dark:hover:text-primary-100"
                  >
                    {t("login.forgotPassword")}
                  </Link>
                </div>

                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-none border border-primary-500 bg-primary-500 px-5 py-3 text-sm font-semibold text-white shadow-sm transition hover:border-primary-600 hover:bg-primary-600 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary-500/30 disabled:cursor-not-allowed disabled:opacity-50"
                >
                  {isSubmitting ? (
                    <>
                      <LoaderMini size={16} color="currentColor" />
                      <span>{t("login.submit")}</span>
                    </>
                  ) : (
                    t("login.submit")
                  )}
                </button>
              </form>

              {isGoogleAuthConfigured() ? <AuthSocialDivider /> : null}

              {isGoogleAuthConfigured() ? (
                <>
                  <GoogleSignInButton
                    mode="login"
                    loading={googleBusy}
                    onSuccess={handleGoogleSignIn}
                    onError={() =>
                      gooeyToast.error(t("login.errorTitle"), {
                        description: t("login.errorFallback"),
                      })
                    }
                  />

                  <p className="mt-3 text-center text-xs text-light-muted dark:text-dark-muted">
                    {t("login.googleHint")}
                  </p>
                </>
              ) : null}

            </div>
          </div>
        </div>
      </AuthLoginBackdrop>
    </GuestOnly>
  );
}
